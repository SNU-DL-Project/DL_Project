...
###구현해야할 것
# (1) SRGAN_G dataload
# (2) SRGAN cfg에서 hyperparams(맨 앞부분) -> 일단 없애고 생각. 필요X
...
class SRGANGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, (blocks, module) in enumerate(zip(self.block_list[1:], self.module_list)):
            if blocks["type"] in ["convolutional", "upsampleblock", "residualblock"]:
                x = module(x)
    ...
    #G는 BatchNorm/conv 이외에 PReLU, Residualblock(Prelu 포함)을 구현해야 한다.
    ...
    def save_srganG_weights(self, path, cutoff=-1):
        """
...
class SRGANDiscriminator(nn.Module):
    ...
    #D는 그냥 넣어도 된다. BatchNorm/Conv 밖에 없음
    ...
    def save_srganD_weights(self, path, cutoff=-1):
        """
            @:param path    - path of the new weights file
            @:param cutoff  - save layers between 0 and cutoff (cutoff = -1 -> all are saved)
        """
...
'''
def load_srgan_model(model_path_D, model_path_G, weights_path=None):
