...
class SRGAN_train(Default_train):
    def __init__(self, model_D, model_G, data_config):
    ...
    def _propagation_srgan(self, real_imgs, generator_criterion):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def _optim_update_srgan(self, batches_done):
        ...
        if batches_done % self.hyperparams['subdivisions'] == 0:
            ...
            self.logger.scalar_summary("train/learning_rate_D", lr_D, batches_done)
            self.logger.scalar_summary("train/learning_rate_G", lr_G, batches_done)
    ...
    def _logging_srgan(self, batches_done, loss_score):
        """
        ...
        if self.verbose:
            print('\n', AsciiTable(
                [
                    ["Type", "Value"],
                    ["D loss", float(loss_score[0])],
                    ["G loss", float(loss_score[1])],
                    ["D score", float(loss_score[2])],
                    ["G score", float(loss_score[3])],
                ]).table, sep='')

        # Tensorboard logging
        tensorboard_log = [
            ("train/D_loss", float(loss_score[0])),
            ("train/G_loss", float(loss_score[1])),
            ("train/D_score", float(loss_score[2])),
            ("train/G_score", float(loss_score[3]))]
        self.logger.list_of_scalars_summary(tensorboard_log, batches_done)
    ...
    def run(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            for batch_i, (_, real_imgs, _) in enumerate(tqdm.tqdm(self.train_dataloader, desc=f"Training Epoch {epoch}")):
                ...
                loss_score = [G_loss.item(), G_loss.item(), fake_out.item(), fake_out.item()]
...
class SRRES_train(Default_train):
    def __init__(self, model_srres, data_config, upscale=2):
    ...
    def _train_one_step_srres(self, real_imgs):
        ...
        lr_imgs = sr_downsample(real_imgs, self.hyperparams['batch'])
        lr_imgs = lr_imgs.to(device, non_blocking=True)
        real_imgs = real_imgs.to(device, non_blocking=True)
