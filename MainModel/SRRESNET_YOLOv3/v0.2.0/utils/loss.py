...
class GeneratorLoss(nn.Module):
    ...
    def forward(self, out_labels, out_images, target_images):
        ...
        #return image_loss + 0.1 * adversarial_loss + 2e-1 * tv_loss
        #return image_loss + 0.001 * adversarial_loss + 2e-8 * tv_loss
        print(255 * 255 * image_loss, tv_loss)
        return 255 * 255 * image_loss + adversarial_loss + tv_loss
        #print(image_loss, adversarial_loss, tv_loss)
        #return image_loss + 0.001 * adversarial_loss + 0.006 * perception_loss + 2e-8 * tv_loss
'''

def compute_srres_loss(fake_imgs, real_imgs):
    ...
    # Image Loss
    image_loss = MSE_loss(fake_imgs, real_imgs)
    # TV Loss
    tv_loss = TV_loss(fake_imgs)

    const_image = 30.0
    const_tv = 10.0
    image_loss = image_loss * const_image
    tv_loss = tv_loss * const_tv
    loss = image_loss + tv_loss

    return loss, [image_loss, tv_loss]

class SRRESLoss(nn.Module):
    def __init__(self):
        super(SRRESLoss, self).__init__()
        #vgg = vgg16(pretrained=True)
        #loss_network = nn.Sequential(*list(vgg.features)[:31]).eval()
        #for param in loss_network.parameters():
        #    param.requires_grad = False
