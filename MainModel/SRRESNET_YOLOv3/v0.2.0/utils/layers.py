...
# Upsample Block, Residual Block은 쓰지 않는다. (data load 및 pruning의 문제)
'''
...
def block_to_layer(module_defs):
    ...
    for module_i, module_def in enumerate(module_defs[1:]):
        ...
        if module_def["type"] == "convolutional":
            ...
            try:
                upsamplescale = int(module_def["upsamplescale"])
                filters = filters // (upsamplescale ** 2)
            except:
                module_def["upsamplescale"] = ''
                upsamplescale = 0

            if upsamplescale:
                modules.add_module(f"PixShuf_{module_i}", nn.PixelShuffle(upsamplescale))
